...
const BASE_POINT = 1; // 성공 시 제공되는 포인트
const POINT_MULTIPLIER = 1; // 레벨별 포인트 배수

const BASE_TIME = 10 * 1000; // 제공 시간
const MAX_TIME = 5 * 60 * 1000; // 최대 시간
...
const LEVEL_POINT_MULTIPLIER = true; // 레벨에 따라 포인트 배수 적용 여부 - true면 레벨 * POINT_MULTIPLIER, false면 BASE_POINT
...
function onTileClick(e) {
  ...
  if (pattern[userInput.length - 1] === idx) {
    // 레벨별 포인트 배수 적용 여부에 따라 점수 계산
    point += LEVEL_POINT_MULTIPLIER ? level * POINT_MULTIPLIER : BASE_POINT;
