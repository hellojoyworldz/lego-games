...
const MAX_TIME = BASE_TIME; // 최대 시간을 기본 시간과 동일하게 설정
const TIME_PENALTY = 5 * 1000; // 틀릴 때 감소
const TIME_REWARD = 3 * 1000; // 맞출 때 증가
const SHOW_INTERVAL = 150; // 캐릭터 보여주는 간격
const CLICK_ANIMATION_DURATION = 200; // 클릭 애니메이션 지속 시간
const WRONG_ANIMATION_DURATION = 400; // 틀린 표시 지속 시간
const NEXT_LEVEL_DELAY = 300; // 다음 레벨로 넘어가는 지연 시간
const RETRY_DELAY = 400; // 재도전 지연 시간
const CHARACTER_IMAGE = "circle10.png"; // 캐릭터 이미지 파일명
...
function showHint() {
  ...
  if (nextIdx !== undefined) {
    ...
    const characterBox = tile.querySelector(".character-box");
    if (
      characterBox.classList.contains("show") &&
      !characterBox.classList.contains("darken")
    ) {
...
function onTileClick(e) {
  ...
  setTimeout(
    () => characterBox.classList.remove("clicked"),
...
function updateTimerDisplay() {
  ...
  timerSpan.textContent = `${String(m).padStart(2, "0")}:${String(s).padStart(
    2,
    "0"
  )}`; // 분도 2자리로 표시
}
